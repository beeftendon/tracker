...
void initialize_grbl(System::IO::Ports::SerialPort^ arduino_port)
{
	...
	serial_response = arduino_tx_rx(arduino_port, "$110=75000"); // X max rate (mm/min)
	System::Console::WriteLine("$110=5000, " + serial_response);

	serial_response = arduino_tx_rx(arduino_port, "$111=75000"); // Y max rate (mm/min)
	System::Console::WriteLine("$111=5000, " + serial_response);

	serial_response = arduino_tx_rx(arduino_port, "$120=500"); // X accel (mm/s^2)
